# FILE: src/maiming/infrastructure/rendering/opengl/_internal/passes/player_model_pass.py
...
class PlayerModelPass:
    ...
    def initialize(self, *, world_prog: ShaderProgram, shadow_prog: ShaderProgram, mesh: MeshBuffer) -> None:
    ...
    def draw_world(self, *, pose: PlayerModelPose, view_proj: np.ndarray, light_view_proj: np.ndarray, sun_dir: Vec3, debug_shadow: bool, shadow_enabled: bool, shadow: ShadowParams, shadow_info: ShadowMapInfo) -> tuple[int, int]:
        if self._world_prog is None or self._mesh is None:
            return (0, 0)
        ...
        shadow_sampling_ok = bool(shadow_enabled and shadow_info.ok and int(shadow_info.tex_id) != 0 and int(shadow_info.inst_count) > 0)
        ...
        with GLStateGuard(capture_framebuffer=False, capture_viewport=False, capture_enables=(GL_BLEND, GL_DEPTH_TEST, GL_CULL_FACE), capture_cull_mode=False, capture_polygon_mode=False):
            ...
            self._world_prog.set_mat4("u_viewProj", view_proj.astype(np.float32, copy=False))
            self._world_prog.set_mat4("u_lightViewProj", light_view_proj.astype(np.float32, copy=False))
            self._world_prog.set_vec3("u_sunDir", float(sun_dir.x), float(sun_dir.y), float(sun_dir.z))
            self._world_prog.set_vec3("u_color", float(self._color.x), float(self._color.y), float(self._color.z))
