...
_CYTHON_MODULES: tuple[str, ...] = (
    "maiming.core.geometry.intersection",
    "maiming.core.grid.voxel_dda",
    "maiming.core.math.view_angles",
)
...
def _base_extensions() -> list[Extension]:
    return [
        Extension(
            name=str(mod),
            sources=[_module_source_path(str(mod))],
        )
        for mod in _CYTHON_MODULES
    ]
...
def _cythonized_extensions() -> list[Extension]:
    try:
        ...
        raise RuntimeError(
            "Cython is required for the explicit local native-extension build path. "
            "Run `python -m pip install -e \".[dev]\" --no-build-isolation` first, "
            "then rerun `python .\\setup.py build_ext --inplace`."
        ) from exc
